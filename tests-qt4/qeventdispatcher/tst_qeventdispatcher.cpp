...
#define FIND_TIMERS() \
        do { \
            ...
            for (int i = 0; i < registeredTimers.count(); ++i) { \
                ...
                if (timerInfo.first == preciseTimerId) { \
                    QCOMPARE(timerInfo.second, int(PreciseTimerInterval)); \
                    foundPrecise = true; \
                } else if (timerInfo.first == coarseTimerId) { \
                    QCOMPARE(timerInfo.second, int(CoarseTimerInterval)); \
                    foundCoarse = true; \
                } else if (timerInfo.first == veryCoarseTimerId) { \
                    QCOMPARE(timerInfo.second, int(VeryCoarseTimerInterval)); \
    ...
    int preciseTimerId = eventDispatcher->registerTimer(PreciseTimerInterval, this);
    int coarseTimerId = eventDispatcher->registerTimer(CoarseTimerInterval, this);
    int veryCoarseTimerId = eventDispatcher->registerTimer(VeryCoarseTimerInterval, this);
